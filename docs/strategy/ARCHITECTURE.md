...
> - **NOW BUILT (2026-08-06)**: `incident_evidence` (WP-2.6 - `010_incident_evidence.sql`), `device_state_history` and `link_state_history` (WP-2.5 - `011_state_history.sql` with diff-on-write L1 caching), and 48-hour event retention window (WP-2.4).

...
> - **Phase 2 correlation engine + incidents + Alerts UI already exist** (`backend/shared/correlation/`, `backend/api/routes/incidents.py`, `frontend/src/app/correlation/page.tsx`) — this doc describes them as future. The identity defect (3.1%) is fixed via WP-1. The edge-direction defect is fixed via WP-2.1 (`009_links.sql`). The incident identity & evidence accumulation defect is fixed via WP-2.2 (deterministic fault fingerprint `(site|root_device|category)`, SQL array union for evidence accumulation, severity escalation, terminal recurrence handling).
...
> - **PHASE 2 IS 100% COMPLETE (2026-08-06)**: WP-2.1 (edge direction), WP-2.2 (incident identity & merge), WP-2.3 (garbage truncated), WP-2.4 (48h retention), WP-2.5 (`011_state_history.sql` with L1 cache), WP-2.6 (`010_incident_evidence.sql`), WP-2.7 (enrichment from canonical devices/sites), WP-2.8 (loop-protected recursive CTE traversal & suppression), and WP-2.9 (direct collector topology writes).
> - **WP-3.1 & WP-3.3 COMPLETE (2026-08-06)**: Generalized Dual-Tier Cache (`shared/cache.py`) with Single-Flight stampede protection & ISO `as_of` timestamp injection + fixed Mist Clients HTTP 404 endpoint drift (`/clients/search`).
> - Remaining future: `metrics_rollup`, `locations`, `diagnostic_runs`, `llm_calls`.

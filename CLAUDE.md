...
- **Tab/view state must survive refresh.** Every page tab or view toggle uses `useQueryState` (from `@/hooks/use-query-state`) — never plain `useState` for navigation state. URL param is the source of truth. Pattern: split the page into `PageInner` (uses `useQueryState`) + default export wrapping it in `<Suspense>`. See `frontend/src/app/sdwan/page.tsx` and `frontend/src/app/mist/page.tsx` for reference.

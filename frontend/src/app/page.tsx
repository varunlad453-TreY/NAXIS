...
function OrbitalSystem({ eventCount }: { eventCount: number }) {
  return (
    <div className="pointer-events-none select-none absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/4"
      style={{ width: "clamp(280px, 35vw, 560px)", height: "clamp(280px, 35vw, 560px)" }}>
      <div className="absolute inset-0 rounded-full"
        style={{ background: "radial-gradient(circle at 50% 50%, hsl(var(--primary)/0.07) 0%, transparent 70%)" }} />

      {/* rings */}
      <div className="absolute inset-0 rounded-full border border-primary/10" />
      <div className="absolute rounded-full border border-primary/12" style={{ inset: "10%" }} />
      <div className="absolute rounded-full border border-violet-500/8" style={{ inset: "20%" }} />
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div style={{ animation: "naxis-orb-pulse 4s ease-in-out infinite, naxis-float 7s ease-in-out infinite" }}>
          ...
          <div className="h-20 w-20 rounded-full flex flex-col items-center justify-center text-center"
            ...
            <div className="text-2xs font-bold uppercase tracking-[0.2em] text-primary/60 mt-0.5">Events</div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute top-6 right-10 text-right" style={{ animation: "naxis-enter 1s 0.8s both" }}>
        <div className="text-2xs font-bold uppercase tracking-[0.2em] text-foreground-subtle">Platforms</div>
      ...
      <div className="absolute bottom-14 left-6" style={{ animation: "naxis-enter 1s 1s both" }}>
        <div className="text-2xs font-bold uppercase tracking-[0.2em] text-foreground-subtle">Vendors</div>
...
function DataTicker({ eventCount }: { eventCount: number }) {
  ...
  return (
    <div className="flex items-center gap-4 overflow-hidden border border-primary/15 bg-primary/4 px-4 py-2 rounded-lg text-xs font-mono"
    ...
    <Link href={href}
      ...
      <div className="relative flex items-start justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          <span className="rounded-full px-2 py-0.5 text-xs font-bold uppercase tracking-widest"
            style={{ color: `rgb(${accentRgb})`, background: `rgba(${accentRgb},0.1)`, border: `1px solid rgba(${accentRgb},0.25)` }}>
            {tag}
          </span>
          {!active && (
            <span className="rounded-full border border-border/30 px-2 py-0.5 text-xs font-bold uppercase tracking-widest text-foreground-subtle">
              Soon
            </span>
          )}
        </div>
      ...
      <div className="relative space-y-1.5 flex-1">
        <p className="text-2xs font-bold uppercase tracking-[0.25em] text-foreground-subtle">{sublabel}</p>
      ...
      <div className="relative flex items-center justify-between border-t pt-3"
        ...
        {stat ? (
          <div>
            ...
            <span className="ml-1.5 text-xs text-foreground-subtle">{stat.label}</span>
          </div>
        ) : (
          <span className="text-xs text-foreground-subtle">Awaiting credentials</span>
        )}
        {active && (
  ...
  return (
    <div className="group grid grid-cols-12 items-center gap-3 px-3 py-2.5 transition-colors hover:bg-primary/4 text-sm border-b border-border/20 last:border-0">
      <div className="col-span-12 flex items-center gap-3 lg:col-span-4">
        ...
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            <span className="font-mono text-2xs text-foreground-subtle">{device.mac}</span>
            {device.model && <span className="rounded border border-violet-400/20 bg-violet-400/10 px-1 text-2xs font-bold uppercase text-violet-400">{device.model}</span>}
          </div>
        </div>
      </div>
      <div className="col-span-6 lg:col-span-3 text-xs">
        <div className="text-2xs font-bold uppercase tracking-wider text-foreground-subtle mb-0.5 flex items-center gap-1"><MapPin className="h-2 w-2" />Site</div>
        <div className="truncate text-foreground">{device.site_name || "—"}</div>
      </div>
      <div className="col-span-6 lg:col-span-2 text-xs">
        <div className="text-2xs font-bold uppercase tracking-wider text-foreground-subtle mb-0.5">IP</div>
        <div className="font-mono text-foreground">{device.ip_address || "—"}</div>
      </div>
      <div className="col-span-6 lg:col-span-2 flex gap-3 text-xs">
        <div>
          <div className="text-2xs font-bold uppercase tracking-wider text-foreground-subtle mb-0.5 flex items-center gap-0.5"><Users className="h-2 w-2" />Clients</div>
          <div className="text-foreground">{device.num_clients}</div>
        </div>
        <div>
          <div className="text-2xs font-bold uppercase tracking-wider text-foreground-subtle mb-0.5 flex items-center gap-0.5"><Clock className="h-2 w-2" />Up</div>
      ...
      <div className="col-span-6 lg:col-span-1 text-right text-xs">
        ...
        {device.firmware_version && <div className="flex items-center justify-end gap-0.5 mt-0.5 text-2xs text-foreground-subtle"><Zap className="h-2 w-2" />{device.firmware_version}</div>}
      </div>
    </div>
  );
}
    ...
    <div className="rounded-lg overflow-hidden border border-border/30">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between px-3 py-2 bg-surface/40 hover:bg-surface/70 transition-colors">
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="text-2xs py-0 h-4">{devices.length}</Badge>
        </div>
...
function InventoryPanel() {
  const { data, isLoading } = useQuery({
    queryKey: ["devices-inline"],
    queryFn: () => api.listDevices({ limit: 500 }),
  });
  const devices = data?.devices ?? [];

  if (isLoading) return <div className="text-sm text-foreground-muted">Loading inventory...</div>;
  if (!devices.length) return <div className="text-sm text-foreground-muted">No devices found. Run the worker to populate inventory.</div>;
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-foreground-subtle font-mono">{filtered.length} devices</span>
        </div>
      </div>
      {isLoading ? (
...
export default function HomePage() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-background">
      ...
      <div className="hidden xl:block absolute right-0 top-0 bottom-0 w-[clamp(280px,35vw,560px)]">
        <OrbitalSystem eventCount={eventCount} />
      </div>

      {/* ── Content ── */}
      <div className="relative mx-auto max-w-screen-2xl px-4 py-12 sm:px-6 xl:px-10 xl:py-16 space-y-12 xl:space-y-16">
        ...
        <section className="max-w-2xl space-y-8" style={{ animation: "naxis-enter 0.8s 0.2s both" }}>
          ...
          <div className="inline-flex items-center gap-3 rounded-full px-4 py-1.5"
            ...
            <span className="text-xs text-foreground-subtle font-mono">v2.0</span>
          </div>

          {/* Headline */}
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.4em] text-foreground-subtle mb-4">
          ...
          <div className="flex flex-wrap items-stretch gap-3">
            {hudStats.map(({ label, value, rgb }) => (
              <div key={label} className="relative flex flex-col px-4 py-3 rounded-lg min-w-[88px]"
                ...
                <div className="text-2xs font-bold uppercase tracking-[0.2em] text-foreground-subtle mt-0.5">{label}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section style={{ animation: "naxis-enter 0.8s 0.4s both" }}>
          <div className="flex items-center gap-3 mb-5">
            ...
            <p className="text-2xs font-bold uppercase tracking-[0.35em] text-foreground-subtle">Platform Observers</p>
        ...
        <section className="border-t border-border/30 pt-8" style={{ animation: "naxis-enter 0.6s 0.9s both" }}>
          <button onClick={() => setShowInventory(s => !s)}
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-foreground-subtle font-mono">all platforms combined</span>
            </div>
            <div className="flex items-center gap-1.5 text-xs font-mono text-foreground-subtle group-hover:text-primary transition-colors">

...
function CollectorRow({ collector, integrationId }: { collector: IntegrationCollectorSummary; integrationId: string }) {
  ...
  return (
    <Link
      href={targetUrl}
      className="group/collector block border-b border-border/40 px-4 py-3 transition-colors hover:bg-background-elevated/20"
    >
      <div className="flex items-start justify-between gap-4">
        {/* Left: collector info */}
        <div className="min-w-0 flex-1 space-y-1">
          <div className="flex items-center gap-2">
            <div className={cn("h-2 w-2", statusDot[collector.status])} />
            <span className="text-sm font-medium text-foreground">{collector.label}</span>
            <span className={cn("inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-[0.12em]", op.color)}>
      ...
      {/* Error message */}
      {collector.message && (
        <div className="mt-2 flex items-start gap-2 border-l-2 border-l-critical pl-3 py-1 text-xs text-critical">
